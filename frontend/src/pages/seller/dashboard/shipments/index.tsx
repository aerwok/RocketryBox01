...
const ShipmentsTable = ({
  ...
  const getSortValue = (shipment: Shipment, key: string): any => {
    switch (key) {
      ...
      case 'amount':
        return parseFloat(shipment.orderId?.total || '0');
      case 'courier':
...
const getOrderDate = (shipment: Shipment): string => {
  return new Date(shipment.createdAt).toLocaleDateString('en-IN');
};
...
const getAmount = (shipment: Shipment): string => {
  return shipment.orderId?.total ? `₹${shipment.orderId.total}` : '-';
};
...
const getPaymentMethod = (_shipment: Shipment): string => {
  // Since payment method is not in the current response, we'll default to 'Prepaid'
  // You may need to update the backend to include payment method if needed
  return 'Prepaid';
};
